...
void initCover()
{
	...
	if (theAD)
	{
		...
		printf("  本次更新内容:\n\t\t1.添加了查询全局数据的功能\n");
		printf("  \t\t2.优化了UI\n");
		printf("  \t\t3.解决了搜索年份和月份时有概率无法找到的问题\n");
		printf("                                                    --By Artcrk\n");
		printf("                                                    --2025/3/12\n");
		printf("<------------------------------ #公告# ---------------------------->\n");
...
void showALL()
{
	int c = 0;
	SetYellow
	printf("     _Year/Mo/Da_ \t _=AC=_  _=Rate=_ \n");
	for (c = 0; c < MaxPtr; c++)
	{
		printf("日期:_%04d/%02d/%02d_ \t __%02d__   __%02d__ \n",dayc[c][0], dayc[c][1], dayc[c][2], dayc[c][3], dayc[c][4]);
	}
}
